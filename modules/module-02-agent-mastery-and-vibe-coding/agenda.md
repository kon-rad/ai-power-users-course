...
| 0:12–0:24 | **Build three skills from one brief** | Read the brief; the agent writes `models-research`, `model-switch`, `spend-report`, then runs each once | Paste the brief, answer its questions |
...
| 1:23–1:41 | **7. Build** | `/model-switch coding`, then `frontend-design` executes the plan with the real photos | Follow along |
...
1. Run `/daily-standup` every morning for a week; `/models-research` and `/spend-report`
